    ...
    <div class="layui-tab">
        <ul class="layui-tab-title" id="tab_list">
            <?php foreach ($tab_list as $key => $value): ?>
                <li class=""  value="<?= $value['id'] ?>"><?= $value['title'] ?></li>
            <?php endforeach; ?>
        </ul>
        <br>
        <div class="top">
            <span><?= $this->name?></span>
            <form action="<?= $this->baseurl.'newyearTheme' ?>" method="post" style="float: right">
...
//            var target = $(event.target).html();
            ...
            window.location.href = "<?=base_url()?>/admin/wldetail/newyearTheme?tab_type="+target;
