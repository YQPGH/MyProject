...
class St_upgrade_config extends Content
{
    function __construct()
    {
        $this->name = '配方合成概率配置';
        $this->control = 'St_upgrade_config';
        $this->list_view = 'st_upgrade_config_list'; // 列表页
        $this->add_view = 'st_upgrade_config_add'; // 添加页

        parent::__construct();
        $_SESSION['nav'] = 3;
        $this->baseurl = site_url('admin/St_upgrade_config/');
        $this->load->model('admin/St_upgrade_config_model', 'model');
    }
...
//        $data = trims($_POST['value']);
        ...
        if (empty($data['grade'])||empty($data['rate'])) show_msg('参数不能为空');
