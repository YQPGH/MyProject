...
class Prize_config extends Content
{
    function __construct()
    {
        $this->name = '商品';
        $this->control = 'prize_config';
        $this->list_view = 'prize_config_list'; // 列表页
        $this->add_view = 'prize_config_add'; // 添加页
        parent::__construct();
        $_SESSION['nav'] = 3;
        $this->baseurl = site_url('admin/prize_config/');
        $this->load->model('admin/prize_config_model', 'model');
    }

    // 首页
    public function index()
    {
        if(!permission('SYS_Prize_config','read')) show_msg('没有操作权限！');
    ...
    public function add()
    {
        if(!permission('SYS_Prize_config','write')){
            show_msg('没有操作权限！');
        }
        $value['total'] = 1000;
        $data['value'] = $value;
...
//        $data = trims($_POST['value']);
    ...
    {
        if(!permission('SYS_Prize_config','del')){
            show_msg('没有操作权限！');
        }
        parent::delete();
    }
