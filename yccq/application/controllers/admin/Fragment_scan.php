...
class Fragment_scan extends Content{

    function __construct(){
        $this->name='碎片管理';
        $this->control = 'Fragment_scan';
        $this->list_view = 'fragment_int_list';
        $this->add_view = 'fragment_add';
        parent::__construct();
        $_SESSION['nav'] = 6;
        $this->baseurl = site_url('admin/Fragment_scan/');
...
//        $keywords = check_str($keywords);
        ...
        $sql = "SELECT COUNT(*) as num  FROM zy_fragment_record a LEFT JOIN zy_user b ON a.uid=b.uid WHERE $where";
