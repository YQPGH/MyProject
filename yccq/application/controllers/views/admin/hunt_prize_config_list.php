    ...
    <table class="layui-table" lay-skin="line">
        ...
        <?php foreach ($list as $key => $value): ?>
            <tr>
                <td><?= $value['name'] ?></td>
                <td><?= $value['pass'] ?></td>
                <td>乐币：<?= $value['money'] ?></td>
                <td><?= ($value['shop1']&&$value['shop1_total']) ? $value['type1']:''?></td>
                <td>
                    商品1等级*数量：<?= $value['shop1'] ?>*<?= $value['shop1_total'] ?><br>
                    商品2等级*数量：<?= $value['shop2'] ?>*<?= $value['shop2_total'] ?><br>
                </td>
                <td><?= $value['rate'] ?></td>
                <td><?= $value['add_time'] ?><br><?= $value['update_time'] ?></td>
                <td>
                    <a href="<?= $this->baseurl . 'edit?id=' . $value['id'] ?>">编辑</a>
                    <a href="<?= $this->baseurl . 'delete?id=' . $value['id'] ?>" onClick="return confirm('确定要删除吗？');">删除</a>
                </td>
            </tr>
        <?php endforeach; ?>
        </tbody>
