...
class Base_model extends CI_Model
{
     ...
     * @return array 二维数组
     */
    function leftInner($table,$table2,$inner, $field = '*', $where = array(), $order = 'id DESC', $limit = 20, $offset = 0){
        $query = $this->db->select($field)->from($table)->join($table2, $inner,'left')
            ->where($where)->order_by($order)->limit($limit, $offset)->get();
     ...
     * @return array 二维数组
     */
    function group_lists($table,$field='*',$where=array(),$group,$order,$limit=100 ){

    $query = $this->db->select($field)->from($table)->where($where)->group_by($group)->order_by($order)->limit($limit)->get();
