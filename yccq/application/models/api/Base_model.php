...
class Base_model extends CI_Model
{
     ...
     * 获取任意字段记录
     *
     * @param string column 要查询的字段  'title, content, date'
     * @param array where  查询条件     array('name =' => $name, 'id <' => $id, 'date >' => $date)
     * @param string table  表名
     * @param string type   0 返回的是一维数组，1返回的是二位数组
     */
    function column_sql($column,$where,$table,$type=0){
    ...
    function column_order_sql($column,$where,$table,$order_file,$order,$limit,$offet=0,$type=0){
        $this->db->select($column);
        $this->db->where($where);
        $this->db->order_by($order_file, $order);
     ...
     * @return array 一维数组
     */
    function get_row($table,$field='*',$where,$limit = 100 ){
        $query = $this->db->select($field)->from($table)->where($where)->limit($limit)->get();
     ...
     * @return array 二维数组
     */
    function lists_sql($sql, $data=null)
    {
        $query = $this->db->query($sql, $data);
