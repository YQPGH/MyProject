...
//        $sql = "select * from `$this->table` WHERE uid=? AND team_id=?";
//        $list = $this->db->query($sql,[$uid,$row['team_id']])->result_array();
//
//        sendMsg($list);
//        return $list;
    }
...
//方法一
    function createSharePng()
    {

        $im = imagecreatetruecolor(640, 700) or die("不能初始化新的 GD 图像流");//创建一张空白图像
        $_bg_color = imagecolorallocate($im, 255,255,255); //创建颜色，返回颜色标识符
        imagefill($im, 0, 0, $_bg_color); //初始化图像背景为$_bg_color
        $bigImgPath = base_url()."static/bg.png";

        $bg=imagecreatefromstring(file_get_contents($bigImgPath));//获取图片
        ...
        imagecopymerge($im,$bgsf,0,0,0,0,640,300,100);//复制合成
        $_text_color = imagecolorallocate($im, 0,0,0);//文字颜色

        $fontpath= 'D:/WWW/yccq/static/msyh.ttf';//字体文件路径
//        /var/www/ Linux绝对路径
        $str = "username";
        $fontSize = 30;
        $im=$this->textcl($im,$_text_color,$str,$fontSize,$fontpath,330,'');//处理多行文字

        $description = '这是测试';
        $desfontSize = 28;
        $im=$this->textcl($im,$_text_color,$description,$desfontSize,$fontpath,410,'      ');

//        $ewm = base_url()."static/leaf_bg_01.png";
