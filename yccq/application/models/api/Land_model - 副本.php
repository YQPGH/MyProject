...
//print_r($temp);exit;
//            if(!is_numeric($temp['type'])) $arr[$temp['type']]++;
            if($temp['is_use']){
                unset($temp['is_use']);
                unset($temp['type']);
...
//        $key = empty($arr)?[0]:array_keys($arr);
//        $return_arr['type'] = $key[0];
...
//        if (!$affected_rows) t_error(4, '采集烟叶出问题了，请稍后再试');
      ...
      if($yanye_shopid && $is_use==0) {
        ...
        $return_arr = array('id'=>$land_id,'yanye_shopid'=>$yanye_shopid,'is_use'=>$is_use,'suipian'=>$status);
