...
class Qixi_model extends Base_model
{
    ...
    function sign($uid)
    {
        ...
        if(!$time) t_error(6,'活动已结束');
        // 判断今天是否已经签到
        $last_row = $this->table_row('zy_qixi_sign',['uid' => $uid]);
        if ($last_row)
        {
        ...
        $this->db->trans_start();
        // 写入签到表
        $this->table_insert('zy_qixi_sign',[
            'uid' => $uid,
            'add_time' => t_time(),
        ]);
        ...
        // 奖励
        $prize = $this->db->query("select shopid,shop_num,json_data  from zy_sign_prize WHERE title=? AND type1=?;",['qixi',$result['sign_total']])->row_array();
        $num = json_decode($prize['json_data']);
        $prize['num'] = $num->num;
        unset($prize['json_data']);

        $this->update_value($uid,$prize['num']);
    ...
    function list_my($uid)
    {
        ...
        //获取每日签到的奖励
        $prize = $this->db->query("select shopid,shop_num,json_data num  from zy_sign_prize WHERE title=?;",['qixi'])->result_array();
        foreach($prize as $v)
        {
    ...
    function get_prize($uid)
    {
        ...
        // 事务开始
        $this->db->trans_start();
        $activi_time = model('user_model')->query_holiday_time('qixi');
        $sql = "select COUNT(*) as num from zy_qixi_prize_record WHERE  uid=? AND type=? and from_unixtime(add_time)>=?";
        $count =  $this->db->query($sql,[$uid,1,$activi_time['start_time']])->row_array();
...
//            }
    ...
    function prize_record($uid)
    {
        ...
        $list = $this->lists_sql("SELECT p.*,c.shop1 shop,c.shop1_total shop_num FROM zy_qixi_prize_record p,zy_prize c
                                  WHERE p.`pid` = c.`id` AND c.type1='qixi_prize' AND p.uid=? and from_unixtime(p.add_time)>=?
                                  ORDER BY p.type DESC,p.add_time DESC LIMIT 30", [$uid,$activi_time['start_time']]);
...
//                    $row = $this->column_sql('*',['pid'=>$v['id']],'zy_message',0);
                    $sql = "select * from zy_message WHERE uid=? AND  pid=? and from_unixtime(add_time)>=?";
                    $row  = $this->db->query($sql,[$uid,$v['id'],$activi_time['start_time']])->row_array();
