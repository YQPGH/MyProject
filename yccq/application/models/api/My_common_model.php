...
//			$query = $this->db->query ( "UPDATE set zy_active_blacklist ReleaseTime=".$data['ReleaseTime']." , ReleaseUid=".$data['ReleaseUid']."  where ActiveBlackID in ($ids)" );
            ...
            $where = "ActiveBlackID in ($ids)";
            $this->db->where($where);
            $this->db->update('zy_active_blacklist', $data);
        }
...
//        $is_exists = $this->db->query("SELECT * FROM zy_active_blacklist WHERE Openid='{$data['Openid']}' and ActiveID={$data['ActiveID']} and ReleaseUid is NULL ")->row_array();
      $sql = "SELECT * FROM zy_active_blacklist WHERE Openid=? and ActiveID=? and ReleaseUid is NULL ";
        $is_exists = $this->db->query($sql, array($data['Openid'], $data['ActiveID']))->row_array();
        if(!$is_exists){
...
//添加公共用户表
	...
	**/
	function addErrLog($Openid, $ChannelID, $ActiveID, $RoomID, $Content, $Type = 0){
		...
		$data['Ip'] = ip();
		$data['ComeUrl'] = $_SERVER['HTTP_REFERER'] ? $_SERVER['HTTP_REFERER'] : '';
...
//			$Isexists = $this->db->query("SELECT * FROM zy_sys_log WHERE Controller = '{$data['Controller']}' AND Method = '{$data['Method']}' AND {$data['AddTime']}-AddTime < 60 AND UID = {$data['UID']}")->row_array();
			$sql = "SELECT * FROM zy_sys_log WHERE Controller = ? AND Method = ? AND {$data['AddTime']}-AddTime < ? AND UID = ?";
            $Isexists = $this->db->query($sql, array($data['Controller'], $data['Method'],60,$data['UID']))->row_array();
            if($Isexists){
...
//        $ruleset = $res->row_array();
        $sql = "SELECT RuleSet FROM zy_gamelist_rule WHERE RuleSign = ? and ChannelID = ? and ActiveID = ? and RoomID = ?";
        $ruleset = $this->db->query($sql, array($RuleSign, $ChannelID, $ActiveID,$RoomID))->row_array();
...
//		$isexit = $this->db->query("select *   from zy_racedog_wx_token where type='ticket' ")->row_array();
	    $sql = "select *   from zy_racedog_wx_token where type=? ";
        $isexit = $this->db->query($sql, array('ticket'))->row_array();
...
//	    $isexit = $this->db->query("select *   from zy_racedog_wx_token where type='token' ")->row_array();
        $sql = "select *   from zy_racedog_wx_token where type=? ";
        $isexit = $this->db->query($sql, array('token'))->row_array();
	    if ($isexit['expire_time'] < time()) {
...
//        $Resources_arr = $this->db->query("SELECT VarName,ReSrc FROM zy_gamelist_resources WHERE  ActiveID = {$ActiveID}")->result_array();
		$sql = "SELECT VarName,ReSrc FROM zy_gamelist_resources WHERE  ActiveID = ?";
        $Resources_arr = $this->db->query($sql, array($ActiveID))->result_array();
        $Resources = array();
