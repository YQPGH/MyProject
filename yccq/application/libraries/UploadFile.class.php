...
class UploadFile
{
    // $max_size 文件大小，M, $dir_name 文件类型和文件夹
    public function upload($file_name, $max_size = 5, $dir_name = 'image') {
        //最大文件大小 100M
...
//定义允许上传的文件扩展名
        $ext_arr = array(
            ...
            'flash' => array('swf', 'flv'),
            'media' => array('swf', 'flv', 'mp3', 'wav', 'wma', 'wmv', 'mid', 'avi', 'mpg', 'asf', 'rm', 'rmvb','jpg'),
            'file' => array('doc', 'docx', 'xls', 'xlsx', 'ppt', 'txt', 'zip', 'rar', 'gz', 'bz2','gif', 'jpg', 'jpeg', 'png', 'bmp'),
        );
...
//有上传文件时
        if (empty($_FILES) === false) {
            //原文件名
            $file_name1 = $_FILES[$file_name]['name'];
            //服务器上临时文件名
        ...
        return $this->alert('文件上传出错了，请联系管理员。');
    }
